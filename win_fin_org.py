...
class input_stati_fin(CTk.CTkFrame):
    def __init__(self, master, arr_org, arr_stat, _list_):
        super().__init__(master, width=1150, height=550)
        #Организация
        ...
        self.money.grid(row=1, column=4, padx=(5,5), pady=(5,5))
        _list_.append(self.money)
...
class win_fin_org(CTk.CTk):
    def __init__(self, id_p):
        ...
        self.entry_organis.grid(row=1, column=0, padx=(5,5), pady=(5,15))
        self.entr_org = CTk.CTkEntry(master=self)
        self.entr_org.grid(row=2, column=0, padx=(5,5), pady=(5,15))
        self.but_input_org = CTk.CTkButton(master=self, text="Добавить организацию", command=self.input_fin_org)
        self.but_input_org.grid(row=3, column=0, padx=(5,5), pady=(5,90))

        self.ttle2 = CTk.CTkLabel(master=self, text="Введите данные по статьям", bg_color="#E63946")
        self.ttle2.grid(row=4, column=0, padx=(5,5), pady=(5,15))
        self.win_input_st = input_stati_fin(self, self._arr_org, arr_stat, self._list_) 
        self.win_input_st.grid(row=5, column=0, padx=(5,5), pady=(5,15))
        self.but_input_st = CTk.CTkButton(master=self, text="Ввод данных", command=self.input_stati_fin)
    ...
    def input_fin_org(self):
        ...
        if match_org:
            ...
            _arr_org = sql.take_nazv_fin_org(self.id_p)
            for el in _arr_org:
            ...
            if prov_nazv:
                ...
                mb.showinfo("Успешно!", "Финансовая организация была добавлена!")
            else:
                mb.showerror("Ошибка!", "Такая организация уже существует")
        else:
            mb.showerror("Ошибка!", "Неверно введено название")

    def input_stati_fin(self):                                      #self.org, self.stat, self.mnt, self.yr, self.ds
        pattern = r"^\d+$"
        prov = True
        year = re.match(pattern, self._list_[3].get())
        if not year:
            prov = False
            mb.showerror("Ошибка!", "Неверно введено значение года")
        ds = re.match(pattern, self._list_[4].get())
        if not ds:
            prov = False
        ...
        if prov:
            ...
            #Добавляем статью 
            sql.input_stati_finorg(id_f_org, id_st_4, self._list_[2].get(), self._list_[3].get(), self._list_[4].get()) 
